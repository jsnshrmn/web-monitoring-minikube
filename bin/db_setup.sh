...
kubectl exec ${pod} bundle exec rake db:create || return 1
kubectl exec ${pod} bundle exec rake db:schema:load || return 1
...
echo "User.create(email: 'annotation-bot@example.com', password: 'PASSWORD', admin: true, confirmed_at: Time.now)" | kubectl exec -it ${pod} bundle exec rails console
